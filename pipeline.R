...
df_model <- df_model %>%
  ...
  mutate(Fws_cat = ifelse(Fws >= .95, 1, 0))
hist(df_model$Fws)
...
set.seed(123)
nfolds <- 10
form_fixed = resistant3 ~ Fws_cat + country
form_randint = resistant3 ~ Fws_cat + (1|country)
form_randslope = resistant3 ~ Fws_cat + (Fws_cat|country)
...
cv_data %>%
  ...
  geom_boxplot()+
  theme_minimal()+
  ggtitle('10-fold CV for prediction of >=3 resistances (sensitivies)')
...
cv_data %>%
  ...
  geom_boxplot()+
  theme_minimal()+
  ggtitle('10-fold CV for prediction of >=3 resistances (specificities)')
